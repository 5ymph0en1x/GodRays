...
class RestClient:
	...
	def curl_bitmex(self, path, query=None, postdict=None, timeout=None, verb=None, max_retries=None):
		...
		response = None
		try:
			self.logger.info("Requesting %s to %s with %s", verb, uri, str(postdict))
	...
	def get_trades(self, symbol, start_time, count=500):
		path = 'trade?count={:d}'.format(count)
		path += '&symbol={:s}'.format(symbol)
		path += '&startTime={:s}'.format(start_time)
	...
	def place_order(self, order, max_retries=None):
		"""Create an order."""
	...
	def update_order(self, orderID, qty, price, max_retries=None):
		...
		return self.curl_bitmex(path=path, postdict=postdict, verb="PUT", max_retries=max_retries)