...
class BitMEXClient:
	...
	def __init__(
			self,
			rest_uri="https://www.bitmex.com/api/v1/",
			ws_uri="wss://www.bitmex.com/realtime/",
			symbols=["XBTUSD"],
			api_key=None,
			api_secret=None,
			use_websocket=True,
	...
	def ws_current_position_size(self, symbol):
	...
	def ws_raw_orders_of_account(self):
		return self._select_ws_client('order').orders()
	...
	def ws_raw_balances_of_account(self, currency):
		return self._select_ws_client('margin').funds(currency)

	def ws_balances_of_account_object(self, currency):
		"""
		...
		"""
		withdrawable_balance, wallet_balance = self.ws_raw_balances_of_account(currency)
		return withdrawable_balance, wallet_balance

	def ws_ticker(self, symbol):
		return self._select_ws_client('').get_ticker(symbol)

	def rest_place_order(self, new_order, max_retries=None):
		response = self.rest_client.place_order(new_order, max_retries=max_retries)
	...
	def rest_cancel_all_orders_symbol(self, symbol):
		open_orders = self.ws_raw_open_orders_of_account()
		self.rest_cancel_orders([o['orderID'] for o in open_orders if o['symbol'] == symbol])

	def rest_cancel_all_orders(self):
		open_orders = self.ws_raw_open_orders_of_account()
		self.rest_cancel_orders([o['orderID'] for o in open_orders])
	
	def rest_update_order(self, orderID, qty, price, max_retries=None):
		self.rest_client.update_order(orderID, qty, price, max_retries=None)
	...
	def rest_get_raw_trade_history(self, symbol, start_time, count=500):
		trades = self.rest_client.get_trades(symbol, start_time, count)
		return trades
